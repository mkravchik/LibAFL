...
use winapi::{
    ...
    um::{
        ...
        processthreadsapi::{CreateRemoteThread, PROCESS_INFORMATION},
        winbase::STD_OUTPUT_HANDLE,
        winnt::{IMAGE_DOS_HEADER, IMAGE_NT_HEADERS, MEM_COMMIT, MEM_RELEASE, PAGE_READWRITE},
    },
};
...
pub extern "system" fn DllMain(
    ...
    match fdw_reason {
        winapi::um::winnt::DLL_PROCESS_ATTACH => {
            ...
            let create_process =
                Module::find_export_by_name(Some("kernel32.dll"), "CreateProcessW");
            let create_process = create_process.unwrap();
            if !create_process.is_null() {
                info!(
                    "{}: CreateProcessW addr: {:p}",
                    std::process::id().to_string(),
                    create_process.0
                );
...
fn hook_trigger_func() {
    ...
    if exe_base.is_null() {
        warn!(
            "{}: Failed to get the exe's base address",
            std::process::id().to_string()
        );
    }
    match trigger_func_name {
        "entry_point" => {
            if !exe_base.is_null() {
                set_entry_point_hook(NativePointer(
                    (exe_base.0 as u64 + trigger_func_offset) as *mut c_void,
                ));
            }
        }
        "main" => {
            ...
            if trigger_func_offset != 0 {
                if !exe_base.is_null() {
                    set_main_hook(NativePointer(
                        (exe_base.0 as u64 + trigger_func_offset) as *mut c_void,
                    ));
                }
            } else {
                let func = Module::find_export_by_name(Some("test.exe"), "main");
                let func = func.unwrap();
                if !func.is_null() {
                    // log(format!("main addr: {:?}", std::process::id().to_string(), main).as_str());
                    // set_fuzz_hook(func);
                    set_main_hook(NativePointer(
                        (func.0 as u64 + trigger_func_offset) as *mut c_void,
                    ));
                }
            }
        }
        _ => {
            if trigger_func_offset != 0 {
                if !exe_base.is_null() {
                    set_fuzz_hook(NativePointer(
                        (exe_base.0 as u64 + trigger_func_offset) as *mut c_void,
                    ));
                }
            } else {
                let func = Module::find_export_by_name(Some("test.exe"), trigger_func_name);
                let func = func.unwrap();
                if !func.is_null() {
                    info!(
                        "{}: {} addr: {:p}",
...
unsafe fn start_fuzzing() {
    ...
    let func = Module::find_export_by_name(Some("test.exe"), "fuzz_internal");
    let func = func.unwrap();
    if !func.is_null() {
...
fn set_entry_point_hook(exe_base_addr_ptr: NativePointer) -> i32 {
    ...
    info!(
        "{}: set_entry_point_hook got the addr: {:p}",
        std::process::id().to_string(),
        module_base
    );
    ...
    unsafe {
        ...
        let nt_headers: &IMAGE_NT_HEADERS =
            &*(module_base.offset(dos_header.e_lfanew as isize) as *const IMAGE_NT_HEADERS);
        ...
        entry_point_address =
            module_base.offset(nt_headers.OptionalHeader.AddressOfEntryPoint as isize);
    }
    ...
    match result {
        Ok(org_fn) => {
            info!(
                "{}: Successfully replaced Exe entry point",
                std::process::id().to_string()
            );
            unsafe {
                *ORIGINAL_ENTRY_POINT.lock().unwrap().get_mut() = Some(std::mem::transmute(org_fn));
            }
            0
        }
        Err(e) => {
            warn!(
                "{}: Failed to replace Exe entry point: {}",
                std::process::id().to_string(),
                e
            );
            -1
...
fn set_create_process_hook(create_proc_addr_ptr: NativePointer) -> i32 {
    ...
    //Print the pointer's value as string
    info!(
        "{}: set_create_proc_hook got the addr: {:p}",
        std::process::id().to_string(),
        create_proc_addr_raw
    );
    ...
    match result {
        Ok(org_create_proc) => {
            info!(
                "{}: Successfully replaced create_proc function",
                std::process::id().to_string()
            );
            unsafe {
        ...
        Err(e) => {
            warn!(
                "{}: Failed to replace create_proc function: {}",
                std::process::id().to_string(),
                e
            );
            -1
...
unsafe extern "C" fn create_process_detour(
    ...
    ) == 0
    {
        ...
        let load_library = Module::find_export_by_name(Some("kernel32.dll"), "LoadLibraryA");
        ...
        if h_thread.is_null() {
            ...
            info!(
                "{}: Library should be injected by now!",
                std::process::id().to_string()
            );
    ...
    // Resume the process
    if winapi::um::processthreadsapi::ResumeThread(process_info.hThread)
        == winapi::shared::minwindef::DWORD::MAX
    {
        warn!(
            "{}: ResumeThread failed: {}",
            std::process::id().to_string(),
...
pub extern "C" fn rust_main_hook(argc: i32, argv: *const *const u8, _env: *const *const u8) -> i32 {
    ...
    unsafe {
        ...
        for i in 0..argc {
            let arg = std::ffi::CStr::from_ptr(*argv.offset(i as isize) as *const c_char)
                .to_str()
                .unwrap();
...
#[no_mangle]
pub extern "C" fn rust_fuzz_hook(sample_bytes: *const c_char, sample_size: u32) {
    info!(
        "{}: rust_fuzz_hook called! {:p} {}",
        std::process::id().to_string(),
...
pub extern "C" fn set_fuzz_hook(fuzz_addr_ptr: NativePointer) -> i32 {
    ...
    match result {
        Ok(org_fuzz) => {
            info!(
                "{}: Successfully replaced fuzz function",
                std::process::id().to_string()
            );
            unsafe {
        ...
        Err(e) => {
            warn!(
                "{}: Failed to replace fuzz function: {}",
                std::process::id().to_string(),
                e
            );
            -1
...
pub extern "C" fn set_main_hook(main_addr_ptr: NativePointer) -> i32 {
    ...
    match result {
        Ok(org_main) => {
            info!(
                "{}: Successfully replaced main function",
                std::process::id().to_string()
            );
            unsafe {
        ...
        Err(e) => {
            warn!(
                "{}: Failed to replace main function: {}",
                std::process::id().to_string(),
                e
            );
            -1
