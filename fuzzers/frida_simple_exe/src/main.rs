...
use winapi::{
    ...
    um::{
        ...
        winbase::{
            CREATE_SUSPENDED, INFINITE, STD_ERROR_HANDLE, STD_INPUT_HANDLE, STD_OUTPUT_HANDLE,
        },
...
pub fn main() {
    ...
    let injected_dll_name = std::env::args()
        .nth(2)
        .expect("Missing injected_dll_name argument");
    ...
    if p_memory.is_null() {
        ...
        if unsafe {
            WriteProcessMemory(
                process_handle,
                p_memory,
                injected_dll_name.as_ptr() as *const _,
                dll_name_len,
                null_mut(),
            )
        } == 0
        {
            ...
            let load_library_addr =
                unsafe { GetProcAddress(kernel32_handle, b"LoadLibraryA\0".as_ptr() as *const i8) };
            ...
            let h_thread = unsafe {
                CreateRemoteThread(
                    ...
                    >(target_load_library_addr as *const ())),
                    p_memory,
                    0,
                    null_mut(),
                )
